...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        model.model.shared_entity_relation.weight = torch.nn.Parameter(
            torch.from_numpy(model_kwargs['entity_relation_weight']).type(torch.FloatTensor))
        model.model.encoder.embed_entitys_relations.weight = torch.nn.Parameter(
            torch.from_numpy(model_kwargs['entity_relation_weight']).type(torch.FloatTensor))
        model.model.decoder.embed_entitys_relations.weight = torch.nn.Parameter(
            torch.from_numpy(model_kwargs['entity_relation_weight']).type(torch.FloatTensor))
