...
#         exsrc_len = exe.size(2)
        # print('e size', e.size())
        # exz = torch.cat([e], dim=-1)
        # print('exz size', exz.size())
        
        # exattn_weights = F.leaky_relu(self.attn_fc(exz).squeeze())
        # print('exattn_weights size', exattn_weights.size()) #torch.Size([250, 2])
        # exit()
        ...
        if e.size(0) > q.size(0):
            qent = torch.mean(e,dim=0,keepdim=True)
            print('qent size', qent.size())
            qe = torch.cat([q, qent], dim=2)
        else:
            qe = torch.cat([q, e], dim=2)
            
            
        # e.size:[250, 4, 384] q.Size:[64, 4, 384]
        # assert e.size(0) <= q.size(0)
#         qe = torch.cat([q, e], dim=2)
        ...
        assert ke is not None
        # with open('qek', 'wb') as f:
        #     pickle.dump(qe, f)
        #     pickle.dump(ke, f)
        #     pickle.dump(edge, f)

        # print('qee', qe.size(), ke.size(), edge.size())
        # [24, 64, 64, 64] [24, 250, 250, 64] [24, 250, 250, 64]

        src_len = ke.size(2)
        z = torch.cat([qe, ke, edge], dim=-1)
        attn_weights = F.leaky_relu(self.attn_fc(z).squeeze())  ##leaky_relu
        # nn.Linear(3 * self.head_dim, 1, bias=bias)
        assert attn_weights.size() == (bsz * self.num_heads, tgt_len, src_len)
...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens, embed_entitys_relations):
        super().__init__()
    ...
    def forward(
            self, input_ids, input_entity_relation_ids, attention_mask=None, word_mask = None, word_subword = None, output_attentions=False,
        ...
        entity_relation_embeds = F.dropout(entity_relation_embeds, p=self.dropout, training=self.training)
        entity_relation_embeds = entity_relation_embeds.transpose(0, 1)
        ...
        for i, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                if i < int(self.encoder_layers / 2):
                    ...
                    x, attn = encoder_layer(x, entity_relation_embeds, attention_mask, word_mask, word_subword, output_attentions=output_attentions)
...
class BartDecoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens: nn.Embedding, embed_entitys_relations: nn.Embedding):
        super().__init__()
        self.dropout = config.dropout
...
#             if i < int(config.decoder_layers / 2):
#                 layers.append(DecoderLayer(config))
#             else:
#                 layers.append(DecoderGATLayer(config))
        for i in range(config.decoder_layers):
            layers.append(DecoderLayer(config))
        self.layers = nn.ModuleList(layers)
    ...
    def forward(
        ...
        # # torch.Size([4, 250]) torch.Size([4, 64])
        # input_entity_relation_ids = input_entity_relation_ids[:, :input_ids.size(1)]
        entity_relation_embeds = self.embed_entitys_relations(input_entity_relation_ids)
        entity_relation_embeds = F.dropout(entity_relation_embeds, p=self.dropout, training=self.training)
        entity_relation_embeds = entity_relation_embeds.transpose(0, 1)
...
#             if idx < int(self.decoder_layers / 2):
#                 x, layer_self_attn, layer_past = decoder_layer(
#                     x,
#                     encoder_hidden_states,
#                     encoder_attn_mask=encoder_padding_mask,
...
#         self.model.shared.num_embeddings
        self.shared_entity_relation = nn.Embedding(entity_relation_weight.shape[0], entity_relation_weight.shape[1], padding_idx=padding_idx)
        ...
        self.decoder = BartDecoder(config, self.shared, self.shared_entity_relation)
        self.init_weights()
...
class KGBartForConditionalGeneration(PretrainedBartModel):
    ...
    def __init__(self, config: BartConfig, entity_relation_weight):
        super().__init__(config)
        base_model = KGBartModel(config, entity_relation_weight)
    ...
    def prepare_inputs_for_generation(self, decoder_input_ids, input_entity_relation_ids, past, attention_mask, use_cache, **kwargs):
