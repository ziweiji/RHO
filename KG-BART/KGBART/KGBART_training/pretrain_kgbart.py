...
def main():
    ...
    parser.add_argument("--pretrained_train_path", default=None, type=str)
    parser.add_argument("--pretrained_eval_path", default=None, type=str)
    ...
    if args.do_train:
        ...
        train_dataset = seq2seq_loader.Pretrain_Seq2SeqDataset(
            args.pretrained_train_path, args.entity_id_path, args.relation_id_path,
        ...
        train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=_batch_size, sampler=train_sampler,
                                                       ...
                                                       pin_memory=False)
    if args.do_eval:
        dev_dataset = seq2seq_loader.Pretrain_Seq2SeqDataset(
            args.pretrained_eval_path, args.entity_id_path, args.relation_id_path,
            args.train_batch_size, tokenizer,
            args.max_seq_length, 
            bi_uni_pipeline=bi_uni_pipeline,
            strict=strict)
    ...
    if (recover_step is None) and (args.model_recover_path is None):
        ...
        model = KGBartForConditionalGeneration.from_pretrained(args.bart_model, entity_relation_weight=entity_relation_embedding)
        global_step = 0
    else:
        if recover_step:
    ...
    if args.do_train:
        ...
        for i_epoch in trange(start_epoch, int(args.num_train_epochs) + 1, desc="Epoch",
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Training", position=0, leave=True)):
                ...
                loss_output = model(input_ids, input_entity_relation_ids=input_entity_relation_ids, attention_mask=subword_mask,
