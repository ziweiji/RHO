...
def main():
    ...
    parser.add_argument("--train_triple_path", default=None, type=str)
    parser.add_argument("--eval_triple_path", default=None, type=str)
    parser.add_argument("--train_hist_path", default=None, type=str)
    parser.add_argument("--eval_hist_path", default=None, type=str)
    parser.add_argument("--train_respone_path", default=None, type=str)
    ...
    parser.add_argument("--keep_last_epochs", default=1, type=int, help="Keep the last few epochs.")
    ...
    if args.do_train:
        ...
        train_dataset = seq2seq_loader.Seq2SeqDataset(
            args.train_triple_path,args.train_hist_path, args.train_respone_path, args.entity_id_path, args.relation_id_path,
            batch_size=args.train_batch_size, tokenizer=data_tokenizer,
    ...
    if args.do_eval:
        dev_dataset = seq2seq_loader.Seq2SeqDataset(
            ...
            batch_size=args.eval_batch_size, tokenizer=data_tokenizer,
            max_len_src=args.max_len_src, max_len_tgt=args.max_len_tgt,
            bi_uni_pipeline=bi_uni_pipeline,
            strict=strict)
        if args.local_rank == -1:
    ...
    if (recover_step is None) and (args.model_recover_path is None):
        ...
        if recover_step:
            ...
            global_step = 0
        model = KGBartForConditionalGeneration.from_pretrained(args.bart_model, state_dict=model_recover, entity_relation_weight=entity_relation_embedding)
    if args.local_rank == 0:
        dist.barrier()
    ...
    if args.do_train:
        ...
        for i_epoch in trange(start_epoch, int(args.num_train_epochs) + 1, desc="Epoch",
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Training", position=0, leave=True)):
                ...
                input_ids, word_subword, input_entity_relation_ids, subword_mask, word_mask, decoder_input_ids, decoder_attention_mask, labels = batch
        ...
        with torch.no_grad():
            for step, batch in enumerate(tqdm(dev_dataloader, desc="Evaluating", position=0, leave=True)):
                ...
                # logging for each step (i.e., before normalization by args.gradient_accumulation_steps)
            dev_loss = sum(cur_dev_loss) / float(len(cur_dev_loss))
            print("the DEV loss is {}".format(dev_loss))
